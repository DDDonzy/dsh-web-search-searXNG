 ...
 *
 * Remote deployments should remove this header or configure their trusted
 * proxy policy explicitly. It is kept for compatibility with the original
 * local-Docker use case.
 */
const TRUSTED_FORWARDED_FOR = "127.0.0.1";

/** Map one SearXNG result entry to a normalized source. */
...
/**
 * SearXNG-backed WebSearchProvider. One search is one HTTP GET against
 * `{baseURL}/search?format=json`.
 */
export class SearxngSearchProvider {
  /** Options for the next operation, resolved once per search. */
  resolveOptions;
