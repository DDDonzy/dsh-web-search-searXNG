...
const DEEPSEEK_SEARCH_PROVIDER_ID = "deepseek-official";
...
function apply(ctx, config) {
  let current = () => config;
  const selectProvider = (source) => {
    current = source;
    // WebRuntime resolves the provider at operation time. Its public selection
    // field is the composition seam used by dsh-web; changing it here lets a
    // saved UI toggle take effect without re-registering the provider.
    ctx.web.searchProviderId = source.enabled === true ? SEARXNG_PROVIDER_ID : DEEPSEEK_SEARCH_PROVIDER_ID;
  };
  installSettingsSection(ctx, WEB_SEARCH_SEARXNG_SETTINGS_NAMESPACE, Config, config, {
    setSource: selectProvider,
    onChange: () => {},
  });
